        ...
        for idx, names in _Genshin_Impact_data.WEAPON_NAME.items():
            for n in names:
                ...
                if n not in self._roster:
                    ...
                    logger.warning(f'Genshin_Impact.chara.Roster: 出现重名{n}于id{idx}与id{self._roster[n]}')
        for idx, names in _Genshin_Impact_data.MONSTER_NAME.items():
            for n in names:
                n = util.normalize_str(n)
                if n not in self._roster:
                    self._roster[n] = idx
...
class Chara:
    ...
    def name(self):
        ...
        elif self.id > 300 and self.id < 1000:
            name = _Genshin_Impact_data.MONSTER_NAME[self.id][0]
        else:
    ...
    def icon(self):
        ...
        if self.id > 1000:
            ...
            res = R.img(f'Genshin_monster_icon/{_Genshin_Impact_data.MONSTER_NAME[self.id][0]}.jpg')
            if not res.exist:
                res = R.img(f'Genshin_monster_icon/{_Genshin_Impact_data.MONSTER_NAME[self.id][0]}.png')        
        else:
